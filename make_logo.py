...
def sample_mixture_model(num_points, cache=True):
    if cache:
...
def draw_frame(ax, state, spikes, data, times, gmms, helpers=None):
    ax.cla()
...
if __name__ == "__main__":
    ...
    data, times, gmms = sample_mixture_model(100)
    ...
    # # Draw a single frame
    draw_frame(ax, 1 /4 * np.pi, spikes, data, times, gmms)
    plt.show()
